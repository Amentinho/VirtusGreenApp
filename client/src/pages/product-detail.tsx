...
// Helper function to get CO2 comparison based on value
function getCO2Comparison(co2Value: number): string {
  const range = CO2_COMPARISONS.find(r => co2Value >= r.min && co2Value < r.max);
  if (!range) return "No comparison available";
  
  // Randomly select one example from the range
  const randomExample = range.examples[Math.floor(Math.random() * range.examples.length)];
                      ...
                      <div className="space-y-4">
                        <div className="bg-gradient-to-r from-emerald-50 to-green-50 p-4 rounded-lg border border-emerald-200">
                          ...
                          <p className="text-lg font-semibold text-gray-900 flex items-start gap-2">
                            ...
                            <span>{getCO2Comparison(impact.co2Emissions)}</span>
                          </p>
                        </div>
